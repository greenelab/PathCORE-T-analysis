"""Should be run *after* `setup_pathcore_db.py`.
...
Usage:
    cache_pathcore_edge_pages.py <network> <db-credentials>
        [--features=<n-features>]
        [--samples=<keep-n-samples>] [--genes=<keep-n-genes>]
    cache_pathcore_edge_pages.py -h | --help
...
class EdgeInfo:
    ...
    def get_edge_info(self, pw0_name, pw1_name):
        ...
        for gene in genes_annotated_to_edge:
            if gene in pw0_annotations and not gene in pw1_annotations:
                genes_which_pathway[gene] = 0
            elif gene in pw1_annotations and not gene in pw0_annotations:
