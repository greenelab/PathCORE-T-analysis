...
Usage:
    setup_pathcore_db.py <dataset> <pathway-definitions> <network>
        <enrichment-analysis-dir> <db-credentials>
        [--models=<n-models>]
        [--features=<n-features>]
        [--is-pathcore-example]
    setup_pathcore_db.py -h | --help
...
Options:
    ...
    <enrichment-analysis-dir>   Path to the directory of enrichment analysis
                                ...
                                `build_co_occurrence_network.py` with the
    ...
    --is-pathcore-example       Sample annotations and common names are
                                available for the P. aeruginosa (PA) KEGG
...
import utils_setup_pa_example as utils_pa


def convert_to_gene_object_ids(gene_set, gene_object_id_map):
    """Given a gene set from the metadata file, convert the
    identifiers to the corresponding MongoDB ObjectIds.

    Parameters
    -----------
    gene_set : str
        Semi-colon separated gene identifiers. If the input is not
        a str (e.g. in pandas, NaN), returns an empty list.
...
if __name__ == "__main__":
    ...
    if is_example:
        utils_pa.paeruginosa_gene_naming(
            genes,
    ...
    for network_num, network_filename in enumerate(
        ...
        for signature_info in feature_signatures:
        ...
        network_feature_signatures.insert_many(
            feature_signatures, ordered=True)

        for pathway_info in feature_pathways:
        ...
        network_feature_pathways.insert_many(feature_pathways, ordered=True)

    if is_example:
        db.sample_annotations.drop()
        sample_annotations = db.sample_annotations
        utils_pa.paeruginosa_sample_annotations(
            sample_annotations,
            sample_labels,
